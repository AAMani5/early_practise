...
def shuffle some_array
  ...
  for index in 0..some_array.length-1
    while ((already.include? random_index) || (index == random_index)) # loop to produce unique rand & != i
      if ((index == some_array.length-1) && (already.sort == (0..some_array.length-2).to_a)))
        puts "Am stuck in loop, need to swap here but breaking"
        break
      end
        random_index = rand(some_array.length)
    end

    puts "adding #{random_index} to already processed list"
